...
public class CatController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (GameManager.Instance.GetCurrentState() != GameState.Play
            ...
            && GameManager.Instance.GetCurrentState() != GameState.CutScene)
        {
            _catAgent.speed = 0f;
            return;
        }
    ...
    private void SetPatrolMovement()
    {
        ...
        if (!_catAgent.pathPending && _catAgent.remainingDistance <= _catAgent.stoppingDistance) //kedi gideceği yere vardıysa
        {
            if (!_isWaiting)
            {
                _isWaiting = true;
